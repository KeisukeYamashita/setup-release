...
import {ReposGetLatestReleaseResponseData, ReposGetReleaseAssetResponseData} from '@octokit/types'
...
export class Downloader{
    ...
    async download(): Promise<string> {
        ...
        var release: ReposGetLatestReleaseResponseData
        if (this.latest) {
            const resp = await client.repos.getLatestRelease({owner: this.cfg.owner, repo: this.cfg.repo})
            release = resp.data
        } else {
            const resp = await  client.repos.getReleaseByTag({owner: this.cfg.owner, repo: this.cfg.repo, tag: this.cfg.tag})
            release = resp.data
        }
        ...
        const assetPath = await tc.downloadTool(asset.browser_download_url, '.', this.cfg.token)
